...
public class nhanvienDAL {
	...
	public boolean insert(nhanvien p) {
		...
		if (openConnection()) {
			try {
				// query
				String sql = "insert into nhanvien(MANV, TENNV,DIACHI,GIOITINH,NGAYSINH) values (?, ?, ?,?,?)";
	...
	public boolean update(nhanvien p) {
		...
		if (openConnection()) {
			try {
				// query
				String sql = "update nhanvien set TENNV=?, DIACHI=?, GIOITINH=?, NGAYSINH=?  where MANV = ?";
				PreparedStatement statement = connection.prepareCall(sql);
	...
	public boolean hasnhanvienCode(int nhanvienCode) {
		...
		if (openConnection()) {
			try {
				// query
				String sql = "select * from nhanvien where MANV=" + nhanvienCode;
				Statement statement = connection.createStatement();
