...
//	List<Product> productList = new ArrayList<Product>();
	...
	public boolean insert(bienlai p) {
		...
		if(openConnection()) {
			try {
	            //query
	            String sql = "insert into bienlai(SO_BL, NGAY_THANH_TOAN, TONG_TIEN, tenBenhNhan) values (?, ?, ?, ?)";
	...
	public boolean delete(int id) {
		...
		if(openConnection()) {
			try {
	            //query
				String sql = "delete from bienlai where SO_BL=?";
	            PreparedStatement statement = connection.prepareCall(sql);
	...
	public List<bienlai> findByName(String Name) {
		...
		if(openConnection()) {
			try {
	            //query
	            String sql = "select * from bienlai where tenBenhNhan like ?";
	            PreparedStatement statement = connection.prepareCall(sql);
	...
	public boolean update(bienlai p) {
		...
		if (openConnection()) {
			try {
				// query
				String sql = "UPDATE bienlai SET NGAY_THANH_TOAN = ?, TONG_TIEN = ?, tenBenhNhan = ? WHERE SO_BL = ?";
				PreparedStatement statement = connection.prepareCall(sql);
	...
	public int getBLMax() {
		...
		if (openConnection()) {
			try {
				// query
				String sql = "select MAX(SO_BL) from bienlai";
				PreparedStatement statement = connection.prepareCall(sql);
